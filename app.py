...
if is_admin and db["stato"] != "setup":
  ...
  with st.sidebar.expander("Modifica Biliardini / Turni"):
    ...
    nuovi_turni = st.number_input(
        "N° Turni", min_value=1, max_value=10, value=db["partite_per_giocatore"]
    )
  ...
  with st.expander(
    ...
    for g_val in range(4):
      with r1_cols[g_val]:
        ...
        if st.button(
            lbl,
            key=f"top_user_g1_{match_id_up}_{g_val}",
            use_container_width=True,
        ):
          m_up["gol1"] = g_val
          salva_dati(db)
    ...
    for g_val in range(4):
      with r3_cols[g_val]:
        ...
        if st.button(
            lbl,
            key=f"top_user_g2_{match_id_up}_{g_val}",
            use_container_width=True,
        ):
          m_up["gol2"] = g_val
          salva_dati(db)
...
if db["stato"] == "setup":
  ...
  if not is_admin:
    ...
    if st.button("🚀 Avvia il Torneo e Genera Calendario"):
      ...
      if len(portieri) < 4 or len(attaccanti) < 4:
        st.error(
            "Inserisci almeno 4 portieri (🥅) e 4 attaccanti (⚽) per poter"
            " giocare."
        )
      else:
...
if db["stato"] == "gironi":
  ...
  for b_num in range(1, num_tavoli + 1):
    if b_num in partite_per_tavolo:
      ...
      if partecipa or is_admin:
        titolo_box = (
            f"⚡ Inserisci Risultato Biliardino {b_num} (Tuo Match in corso!)"
            if partecipa
        ...
        with st.expander(titolo_box):
          ...
          st.markdown(
              "<div style='margin: 14px 0;'></div>", unsafe_allow_html=True
          )
          if st.button(
              "💾 Conferma e Salva Risultato Finale",
  ...
  st.markdown(
      f"### 📢 PROSSIMI IN CODA"
      f" ({min(len(partite_in_coda), num_partite_in_corso)} in attesa):"
  )
...
if db["stato"] == "eliminatorie":
  ...
  if finito_tutto:
      ...
      if tutti_giocati and is_admin:
        if (
          ...
          if st.button(
            ...
            and not any(f["nome"] == "Finali (1°-2° e 3°-4° Posto)" for f in fasi)
        ):
          if st.button(
              "🏁 Genera Finali",
              use_container_width=True,
    ...
    if is_admin:
      if st.button(
          "⬅️ Indietro ai Gironi",
          use_container_width=True,
          key="main_back_gironi",
      ):
        db["stato"] = "gironi"
        salva_dati(db)
        st.rerun()
